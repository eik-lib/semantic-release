...
const messages = {
  ETOKENMISSING: "Verify: ❌ EIK_TOKEN environment variable not found",
  ELEVELINVALID:
    'Verify: ❌ Plugin option "level" must be one of "major", "minor" or "patch"',
  ELOGINFAIL: "Verify: ❌ Unable to log in to Eik server",
};
...
module.exports = async function verifyConditions(options, context, state) {
  ...
  if (!context.env.EIK_TOKEN) errors.push(new Error(messages.ETOKENMISSING));

  // verify options.level is one of "major", "minor" or "patch" if set
  if (!["major", "minor", "patch"].includes(options.level || "patch")) {
    errors.push(new Error(messages.ELEVELINVALID));
  }
  ...
  if (!state.eikToken) errors.push(new Error(messages.ELOGINFAIL));
